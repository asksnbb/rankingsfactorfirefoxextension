import {
    Accordion,
    AccordionItem,
    AccordionTrigger,
    AccordionContent,
} from "@/components/ui/accordion";
import { FileText } from 'lucide-react';
import { Separator } from "@/components/ui/separator"

// Helper: badge bg/text color class based on status
function getStatusBadgeClasses(status) {
    switch (status) {
        case "Excellent":
            return "bg-green-100 text-green-700 border-green-200";
        case "Good":
            return "bg-yellow-100 text-yellow-800 border-yellow-200";
        default:
            return "bg-red-100 text-red-700 border-red-200";
    }
}

// Helper: Search engine opinion for description
function getSearchEngineOpinion(status) {
    switch (status) {
        case "Excellent":
            return "Search engines will likely use and display this description as a high-quality snippet in search results.";
        case "Good":
            return "Search engines can use this description, but it could be improved for maximum impact and CTR.";
        default:
            return "Search engines may generate their own snippet or ignore this description due to its current quality. Revise for best results.";
    }
}

const Description = ({ description }) => {
    if (!description) return null;

    return (
        <Accordion type="single" collapsible className="p-2 space-y-3">
            <AccordionItem
                key={description.key}
                value={`item-${description.key}`}
                className="rounded-xl border shadow-sm transition-transform duration-100 hover:-translate-y-1 hover:shadow-md"
                style={{ borderColor: "#eeeeee", borderWidth: 2 }}
            >
                <AccordionTrigger
                    className="flex items-center px-3 py-2 gap-3 no-underline transition-colors"
                    style={{
                        borderBottom: "none",
                        textDecoration: "none",
                        boxShadow: "none",
                    }}
                >
                    <span className="p-2 rounded-full bg-white shadow-sm ring-1 ring-gray-100 flex items-center">
                        <FileText size={20} className="text-blue-500" />
                    </span>
                    {/* Description label */}
                    <span className="flex-1 font-semibold text-gray-800 text-base capitalize">
                        {description.key}
                    </span>
                    {/* Status badge */}
                    <span
                        className={
                            "ml-2 rounded px-2 py-0.5 text-xs font-semibold border " +
                            getStatusBadgeClasses(description.status)
                        }
                    >
                        {description.status}
                    </span>
                </AccordionTrigger>
                <AccordionContent>
                    <div className="px-3 pb-3 text-sm text-gray-700">
                        <strong>Description:</strong> {description.value}
                    </div>
                    <div className="px-3 pb-3 text-sm text-gray-700">{description.message}</div>
                    <div className="px-3">
                        <Separator className="my-4" />
                    </div>
                    <div className="px-3 pt-1 pb-3 text-sm text-blue-700 italic">
                        <strong>Search Engine Insights:</strong> {getSearchEngineOpinion(description.status)}
                    </div>
                </AccordionContent>
            </AccordionItem>
        </Accordion>
    );
};

export default Description;
